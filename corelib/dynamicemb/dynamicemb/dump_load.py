...
from .dynamicemb_config import dyn_emb_to_torch, torch_to_dyn_emb
from dynamicemb_extensions import DynamicEmbTable
from dynamicemb_extensions import insert_or_assign,count_matched, clear
from dynamicemb_extensions import reserve,erase,export_batch,dyn_emb_rows,dyn_emb_cols,dyn_emb_capacity
...
def debug_check_dynamic_table_is_zero(dynamic_table):
    ...
    score_is_one = True
    if need_dump_score:
        score_is_one = torch.all(scores == 1) # HKV score have a placeholder value , the value is 0 ,so we use scores == 1
    ...
    if value_is_zero and score_is_one:
        ...
        print(f"DynamicEmb Debug:Not all values are zero. Non-zero values found at indices: {non_zero_indices_for_value.tolist()}")
        if need_dump_score:
            non_one_indices_for_score = torch.nonzero((scores.to(torch.int64) != 1))
            print(f"DynamicEmb Debug:Not all scores are one. Non-one scores found at indices = {non_one_indices_for_score} scores = {scores}")
        raise ValueError(f"DynamicEmb Debug:Not all values are zero and not all scores are one. Non-zero values found")
...
def debug_dump(embedding_collections_list, path, table_names, optim, pg):
    def debug_export(dynamic_table, root_path, name):
        ...
        values[:] = 0
        if need_dump_score:
            scores[:] = 1 # HKV score have a placeholder value , the value is 0 ,so we set scores = 1
    ...
    for i, embedding_collection in enumerate(embedding_collections_list):
        ...
        for j, dynamic_emb_module in enumerate(tmp_dynamic_emb_module_list):
            ...
            tmp_tables_dict: Dict[str, DynamicEmbTable] = {name: table for name, table in zip(filtered_table_names, filtered_dynamic_tables)}
...
def debug_load(embedding_collections_list, path, table_names, optim, pg):
    def validate_dynamic_embedding(debug_root_path, dump_name, rank, dynamic_table):
        ...
        keys_in_dynemb = torch.empty(local_max_rows, dtype=key_dtype_in_table, device=device)
        values_in_dynemb = torch.empty(local_max_rows * dim, dtype=value_dtype_in_table, device=device)
        ...
        if need_dump_score:
            ...
            scores_in_dynemb = torch.empty(local_max_rows, dtype=score_dtype_in_table, device=device)
        ...
        d_counter = torch.zeros(1, dtype=torch.uint64, device=device)
        if need_dump_score:
            export_batch(dynamic_table, dyn_emb_capacity(dynamic_table), 0, d_counter, keys_in_dynemb, values_in_dynemb,scores_in_dynemb)
        else:
            export_batch(dynamic_table, dyn_emb_capacity(dynamic_table), 0, d_counter, keys_in_dynemb, values_in_dynemb)
        ...
        sorted_keys_in_dynemb_int64, sorted_indices_in_dynemb_int64 = torch.sort(keys_in_dynemb_int64)

        sorted_values_device = values_device.view(local_max_rows, dim)[sorted_indices_device].view(-1)
        sorted_values_in_dynemb_float = values_in_dynemb_float.view(local_max_rows, dim)[sorted_indices_in_dynemb_int64].view(-1)
        ...
        if sorted_values_device.shape != sorted_values_in_dynemb_float.shape:
            raise ValueError(f"DynamicEmb Debug: Shape mismatch for values in table {dump_name}: {sorted_values_device.shape} (dumped) != {sorted_values_in_dynemb_float.shape} (loaded)")
        ...
        if need_dump_score:
            ...
            sorted_scores_in_dynemb_int64 = scores_in_dynemb_int64[sorted_indices_in_dynemb_int64]

            if sorted_scores_device.shape != sorted_scores_in_dynemb_int64.shape:
                raise ValueError(f"DynamicEmb Debug: Shape mismatch for scores in table {dump_name}: {sorted_scores_device.shape} (dumped) != {sorted_scores_in_dynemb_uint64.shape} (loaded)")

            if not torch.equal(sorted_scores_device, sorted_scores_in_dynemb_int64):
                diff_values = torch.nonzero(sorted_scores_device != sorted_scores_in_dynemb_int64)
                raise ValueError(f"DynamicEmb Debug: Mismatch in scores for table {dump_name} at positions {diff_values.tolist()}")
...
def gather_and_export(
    ...
    while offset < search_capacity:
        keys, values,scores, d_counter = export_keys_values(dynamic_table, offset, device, batch_size)
        ...
        if need_dump_score:
            ...
            gathered_scores_bytes = [torch.empty_like(scores_bytes) for _ in range(world_size)]  if rank ==0 else None
        ...
        if need_dump_score:
            dist.gather(scores_bytes, gather_list=gathered_scores_bytes, dst=0, group=pg)

        if rank == 0:

            if need_dump_score:
                gathered_scores = [tensor.view(torch.uint64) for tensor in gathered_scores_bytes]
            for i in range(world_size):
                ...
                if actual_length > 0:
                    ...
                    if need_dump_score:
                        tmp_gathered_scores = gathered_scores[i][:actual_length].to(torch.uint64).cpu()
...
def load_table(
    ...
    dim = dyn_emb_cols(dynamic_table)
    table_capacity = dyn_emb_capacity(dynamic_table)
    ...
    if need_dump_score:
        ...
        scores = []
        score_dtype = torch.uint64
        scores_read_bytes = batch_size * 8#socre in file always uint64, so is 8
        score_file_size = os.path.getsize(score_path)
    ...
    with open(key_path, 'rb') as fkey, open(value_path, 'rb') as fvalue:
        ...
        while True:
            ...
            if masked_keys.shape[0] > 0:
                ...
                values_tensor = torch.tensor(masked_values, dtype=value_dtype, device=device)
                if need_dump_score:
                    scores_tensor = torch.tensor(masked_scores, dtype=score_dtype, device=device)
                    insert_or_assign(dynamic_table,masked_keys.shape[0],keys_tensor, values_tensor,scores_tensor)
                else:
                    insert_or_assign(dynamic_table,masked_keys.shape[0],keys_tensor, values_tensor)
    ...
    for i, tmp_collection in enumerate(collections_list):
        ...
        for j, dynamic_emb_module in enumerate(tmp_dynamic_emb_module_list):
            ...
            for k, load_name in enumerate(filtered_table_names):
                load_table(filtered_dynamic_tables[k], full_collection_path, load_name, pg=pg , debug_mode=debug_mode)
