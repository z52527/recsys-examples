...
from dynamicemb_extensions import assign,find,erase,reserve,export_batch,dyn_emb_rows,dyn_emb_cols,dyn_emb_capacity
...
def set_score(model: torch.nn.Module, table_score: Union[int, Dict[str, Dict[str, int]]]) -> None:
    ...
    if len(collections_list) == 0:
        warnings.warn("Input model don't have any TorchREC ShardedEmbeddingCollection or ShardedEmbeddingBagCollection module, can't get score!", UserWarning)
        return
    ...
    if not has_dynamic_emb:
        warnings.warn("Input model don't have any Dynamic embedding tables, can't set score!", UserWarning)
        return
    if not set_all_table:
        ...
        for tmp_input_collection_name in table_score.keys():
            if tmp_input_collection_name not in collection_names_in_module:
                warnings.warn(f"sharded module '{tmp_input_collection_name}' specified in table_score not found in the model", UserWarning)
...
def get_score(model: torch.nn.Module)-> Union[Dict[str, Dict[str, int]], None]:
    """Get score for each dyanmic embediing table.
    ...
    if not has_dynamic_emb:
        warnings.warn("Input model don't have any Dynamic embedding tables, can't get score!", UserWarning)
        return
...
def incremental_dump(model: torch.nn.Module, score_threshold: Union[int, Dict[str, Dict[str, int]]], pg: Optional[dist.ProcessGroup] = None) -> Union[Tuple[Dict[str, Dict[str, Tuple[torch.Tensor, torch.Tensor]]], Dict[str, Dict[str, int]]], None]:
    ...
    collections_list: List[Tuple[str,str, nn.Module]] = find_sharded_modules(model, "")
    if len(collections_list) == 0:
        warnings.warn("Input model don't have any TorchREC ShardedEmbeddingCollection or ShardedEmbeddingBagCollection module, can't incremental dump!", UserWarning)
        return
    ...
    if not has_dynamic_emb:
        warnings.warn("Input model don't have any Dynamic embedding tables, can't incremental dump!", UserWarning)
        return
    if not set_all_table:
        #filter the embedding collection 
        collection_names_in_module = set()
        ...
        for tmp_input_collection_name in score_threshold.keys():
            if tmp_input_collection_name not in collection_names_in_module:
                warnings.warn(f"sharded module '{tmp_input_collection_name}' specified in score_threshold not found in the model", UserWarning)
    ...
    for i, tmp_collection in enumerate(collections_list):
        ...
        for j, dynamic_emb_module in enumerate(tmp_dynamic_emb_module_list):
            ...
            if len(filtered_table_names) == 0:
                continue
            #do incremental dump
            tensors, scores = dynamic_emb_module.incremental_dump(dict(zip(filtered_table_names, filtered_thresholds)), pg)
