...
class ScoreAdaptor:
  def __init__(self, evict_strategy: EvictStrategy, dtype: torch.dtype, device: torch.device):
    self.evict_strategy_ = evict_strategy
    self.dtype_ = dtype
    self.device_ = device
    
    min_step = 0
    self.min_score_: int = device_timestamp() if evict_strategy == EvictStrategy.KLru else min_step
  ...
  def next_score(self) -> int:
    ...
    return device_timestamp() if self.evict_strategy_ == EvictStrategy.KLru else next_score
...
@pytest.mark.parametrize("evict_strategy", [EvictStrategy.KLru, EvictStrategy.KCustomized])
@pytest.mark.parametrize("bucket_capacity, batch, capacity, num_iteration, dump_interval", [
  pytest.param(128, 128, 512 * 1024, 8192, 1024, id="Never evict keys from current batch"),
  pytest.param(128, 65536, 512 * 1024, 32, 8),
  pytest.param(128, 1024 + 13, 1024, 12, 3, id="Always evict keys from current batch"),
  pytest.param(128, 1024, 4 * 1024, 32, 4, id="Always evict keys from last dump_interval"),
  pytest.param(512, 512, 512 * 1024, 2048, 256, id="Different bucket capacity")])
def test_dynamicemb_extensions(request, ext_option, score_type, counter_dtype, evict_strategy, 
  ...
  keys = torch.tensor(list(keys_set), dtype=dyn_emb_to_torch(ext_option.key_type), device=device)
  values = torch.empty(bucket_capacity, ext_option.dim, dtype=dyn_emb_to_torch(ext_option.value_type), device=device)
  ...
  dump_vals = torch.empty(num_matched, ext_option.dim, dtype=values.dtype, device=device)
  d_num_matched.fill_(0)
  export_batch_matched(table, init_score, table.capacity(), 0, d_num_matched, dump_keys, dump_vals)
  ...
  for i in range(0, num_iteration, dump_interval):
    for j in range(dump_interval):
      ...
      keys = torch.tensor(list(keys_set), dtype=dyn_emb_to_torch(ext_option.key_type), device=device)
      values = torch.empty(batch, ext_option.dim, dtype=dyn_emb_to_torch(ext_option.value_type), device=device)
      score = score_adaptor.score()
      ...
      insert_and_evict(table, batch, keys, values, score, evict_keys, evict_vals, evict_scores, evict_counter)
      ...
      valid_evict_scores = evict_scores[:num_evict].to(dtype=torch.int64, device="cpu")
      last_evict += torch.sum((valid_evict_scores >= last_score) & (valid_evict_scores < undump_score)).item()
    ...
    print(f"Load factor={load_factor:.3f}, num_dump_more={num_dump_more}, num_dump_less={num_dump_less}, \
    ...
    # update
    last_score, last_insert, last_evict = (undump_score, undump_insert, undump_evict)
    undump_score, undump_insert, undump_evict = (score_adaptor.next_score(), 0, 0)
