...
def test(args):
    ...
    for i in range(num_iterations):
        ...
        with torch.no_grad():
            ...
            hit_counter = 0
            for length, indices_per_table, dict_ in zip(
                lengths, [x.values() for x in sparse_feature.split(segment)], ref_dicts
            ):
                ...
                for j, index in enumerate(indices_per_table):
                    ...
                    expected_diff = float(dict_[index] % 100000)

                    if res_diff.item() != expected_diff:
                        print(
                            "diff: ",
                            res_diff,
                            expected_diff,
