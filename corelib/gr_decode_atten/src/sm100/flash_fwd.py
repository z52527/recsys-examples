...
from ..common.pack_gqa import PackGQA, pack_gqa_layout
...
from ..common.fa_logging import fa_log, fa_printf
from ..common.utils import smid
...
class FlashAttentionForwardSm100:
    ...
    def __init__(
        self,
        config: KernelConfig,
        dtype=None,  # unused on SM100 (inferred from tensor at runtime)
        is_split_kv: bool = False,
        has_beam_sparse: bool = False,  # unused on SM100 (3-kernel only)
        ...
        self.m_block_size = config.tile_m  # SM100 uses m_block_size
        self.n_block_size = config.tile_n  # SM100 uses n_block_size
        ...
        self.mma_tiler_qk = (self.cta_group_size * self.m_block_size, self.n_block_size, self.head_dim_padded)
        self.mma_tiler_pv = (self.cta_group_size * self.m_block_size, self.head_dim_v_padded, self.n_block_size)
        ...
        # has_beam_sparse unused on SM100 (3-kernel path only, no fused beam)
        ...
        _default_enable_ex2_emu = (self.head_dim_padded <= 128 or (self.head_dim_padded == 192 and self.use_2cta_instrs)) and not is_sm103
        ...
        if self.overlap_sO_sQ or self.is_split_kv:
    ...
    def __call__(
        ...
        # Beam sparse fusion parameters (unused on SM100 for now)
        mQ_beam: Optional[cute.Tensor] = None,
        ...
        # Determine split-KV from actual tensor shape (5D = split-KV, 4D = no split).
        # Override self.is_split_kv so downstream kernel code uses the correct path.
        self.is_split_kv = const_expr(len(mO.shape) == 5)
        ...
        KernelConfig.check_type(
            ...
            mCuSeqlensQ.element_type if const_expr(mCuSeqlensQ is not None) else None,
            mCuSeqlensK.element_type if const_expr(mCuSeqlensK is not None) else None,
            mSeqUsedQ.element_type if const_expr(mSeqUsedQ is not None) else None,
            mSeqUsedK.element_type if const_expr(mSeqUsedK is not None) else None,
        )
        ...
        if const_expr(self.enable_ex2_emu):
            ...
            if const_expr(
                self.pack_gqa and self.head_dim_padded > 64
            ):
