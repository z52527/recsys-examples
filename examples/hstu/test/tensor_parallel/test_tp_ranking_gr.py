...
@pytest.mark.parametrize("max_num_candidates", [0, 10])
@pytest.mark.parametrize("optimizer_type_str", ["sgd", "adam"])  # TODO: add adam
@pytest.mark.parametrize("dtype", [torch.bfloat16])
@pytest.mark.parametrize("replicate_batches", [False])  # same batch or various batches
...
def test_tp_gr_ranking_forward_backward_update(
    ...
    tp_model, tp_dense_optimizer, _ = create_model(
        ...
        num_heads=num_heads,
        sequence_parallel=sequence_parallel,  # only native supports SP
    )
    (
        debug_model,
        ...
        num_batches=30
        if optimizer_type_str == "sgd"
        else 60,  # TODO, there are cumulative errors, is this a bug?
