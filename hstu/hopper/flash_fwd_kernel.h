...
template <typename Ktraits, bool Is_causal, bool Is_target, bool Is_context, bool Is_local, bool Has_Rab, bool Is_delta_q, typename TileScheduler, typename Seqlen_traits>
__global__ void __launch_bounds__(Ktraits::kNWarps * cutlass::NumThreadsPerWarp, 1)
    compute_attn_ws(CUTE_GRID_CONSTANT typename CollectiveMainloopFwd<Ktraits, Is_causal, Is_target, Is_context, Is_delta_q, Is_local, Has_Rab, Seqlen_traits>::Params const mainloop_params,
    ...
    using CollectiveMainloop = CollectiveMainloopFwd<Ktraits, Is_causal, Is_target, Is_context, Is_delta_q, Is_local, Has_Rab, Seqlen_traits>;
    ...
    if (warp_group_idx == 0) {  // Producer
        ...
        if (warp_idx_in_warpgroup == 0) {  // Load Q, K, V
            ...
            PipelineState smem_pipe_write_rab = cutlass::make_producer_start_state<MainloopPipeline>();
            ...
            for (auto work_tile_info = scheduler.get_initial_work();
                ...
                if constexpr(Is_target) {
                    seqlen_traits_k.init_h(bidb); // note: use k for delta_q
                }
                if constexpr(Is_context) {
                    seqlen_traits_k.init_c(bidb);
                }
                const int actual_seq_q = seqlen_traits_q.actual_seq_len;
                const int actual_seq_k = seqlen_traits_k.actual_seq_len;
                const int actual_seq_h = Is_target ? seqlen_traits_k.actual_seq_len_h : actual_seq_k;
                const int actual_seq_c = Is_context ? seqlen_traits_k.actual_seq_len_c : 0;
                if (m_block * kBlockM >= actual_seq_q) {
                    continue;
                }
                ...
                const int target_index = (m_block * kBlockM - actual_seq_h) / mainloop_params.target_group_size;

                // calculate n_block_min and n_block_max
                int n_block_min =
                    (!Is_local)
                        ? 0
                        : std::max(0, (m_block * kBlockM + actual_seq_offset - mainloop_params.window_size_left) /
                                        kBlockN);
                if constexpr (Is_context) {
                ...
                if constexpr (Is_causal || Is_local) {
                    n_block_max = std::min(
                        n_block_max,
                        cute::ceil_div((m_block + 1) * kBlockM + actual_seq_offset + mainloop_params.window_size_right,
                                    kBlockN));
                }
                if constexpr (Is_context) {
                ...
                if constexpr (Is_target) {
                    n_masking_block_min = is_jump ? (actual_seq_h + actual_seq_offset + target_index * mainloop_params.target_group_size) / kBlockN : n_masking_block_min;
                }
                if constexpr (Is_context) {
                ...
                n_block_max, n_block_min, n_masking_steps, is_jump, n_block_history,
                shared_storage, scheduler, scheduler_params, work_tile_info, block_coord, work_idx, seqlen_traits_q, seqlen_traits_k);
                ++work_idx;
            }
            collective_mainloop.load_tail(pipeline_k, pipeline_rab, pipeline_v, smem_pipe_write_k, smem_pipe_write_rab, smem_pipe_write_v);
        }
    } else {  // Consumer
        // TODO: Tune the reg num
        ...
        for (auto work_tile_info = scheduler.get_initial_work();
            ...
            const int n_masking_steps = (!Is_causal || is_context) ? 0 : n_masking_block_max - n_masking_block_min;
            if ((Is_causal || Is_local) && n_block_max <= n_block_min) {  // We exit early and write 0 to gO.
                collective_epilogue.store_zero(epilogue_params, shared_storage, threadIdx.x - NumCopyThreads, block_coord, seqlen_traits_q);
                continue;
            }
            collective_mainloop.mma(mainloop_params, pipeline_k, pipeline_rab, pipeline_v, smem_pipe_read_k, smem_pipe_read_rab, smem_pipe_read_v,
                                    tOrO, n_block_max, n_block_min, n_masking_steps,
                                    is_jump, n_block_history,
...
template <typename Ktraits, bool Is_causal, bool Is_local, bool Has_Rab, typename TileScheduler, typename Seqlen_traits>
__global__ void __launch_bounds__(Ktraits::kNWarps * cutlass::NumThreadsPerWarp, 1)
    compute_attn_ws_fp8(CUTE_GRID_CONSTANT typename CollectiveMainloopFwd<Ktraits, Is_causal, false, false, false, Is_local, Has_Rab, Seqlen_traits>::Params const mainloop_params,  CUTE_GRID_CONSTANT typename CollectiveEpilogueFwd<Ktraits, Seqlen_traits>::Params const epilogue_params,
    CUTE_GRID_CONSTANT typename TileScheduler::Params const scheduler_params,
    Seqlen_traits seqlen_traits_q, Seqlen_traits seqlen_traits_k) {
    ...
    using CollectiveMainloop = CollectiveMainloopFwd<Ktraits, Is_causal, false, false, false, Is_local, Has_Rab, Seqlen_traits>;
    using CollectiveEpilogue = CollectiveEpilogueFwd<Ktraits, Seqlen_traits>;
    ...
    if (warp_group_idx == 0) {  // Producer
        ...
        for (auto work_tile_info = scheduler.get_initial_work();
            ...
            collective_mainloop.load_fp8(
                mainloop_params, pipeline_k, pipeline_rab, pipeline_v, pipeline_vt,
        ...
        for (auto work_tile_info = scheduler.get_initial_work();
            ...
            int n_block_max = collective_mainloop.get_n_block_max(
                mainloop_params, m_block, seqlen_traits_q, seqlen_traits_k);
            if constexpr(Is_causal) {
                if(n_block_max <= 0) {  // We exit early and write 0 to gO.
                    collective_epilogue.store_zero(epilogue_params, shared_storage, threadIdx.x - NumCopyThreads, block_coord, seqlen_traits_q);
                    continue;
                }
            }
