...
template <typename Ktraits, bool Is_causal, bool Is_target, bool Is_context, bool Is_delta_q, bool Is_local, bool Has_Rab, typename Seqlen_traits>
struct CollectiveMainloopFwd {
    ...
    // static constexpr bool UseSchedulerBarrier = cutlass::sizeof_bits_v<Element> == 8 ? kHeadDim >= 128 : kHeadDim <= 128;
    ...
    int get_n_block_max(
        ...
        int const actual_seq_offset = Is_delta_q ? seqlen_k - seqlen_q : 0;
        int n_block_max = cute::ceil_div(seqlen_k, kBlockN);
        if constexpr (Is_causal || Is_local) {
            n_block_max = std::min(
                n_block_max,
                cute::ceil_div((m_block + 1) * kBlockM + actual_seq_offset + mainloop_params.window_size_right, kBlockN));
        }
        return n_block_max;
    }
    ...
    int get_n_block_min(
        ...
        if constexpr (Is_local) {
            n_block_min = std::max(
                0, 
                (m_block * kBlockM + actual_seq_offset - mainloop_params.window_size_left) / kBlockN
            );
        }
        return n_block_min;
    }
    ...
    load(Params const& mainloop_params,
        ...
        auto [tRabgRab, tRabsRab] = tma_partition(mainloop_params.tma_load_Rab, _0{}, Layout<_1>{},
                                          group_modes<0, 2>(sRab), group_modes<0, 2>(gRab));  // (TMA, k), (TMA, PIPE)
        ...
        if (lane_predicate) {
            ...
            if constexpr (IntraWGOverlap) {
                ...
                for (int masking_step = 0; n_block > n_block_min; ++masking_step, --n_block) {
                    ...
                    if (is_jump && masking_step == n_masking_steps - 1) { // ensure the load is current.
    ...
    load_fp8(Params const& mainloop_params,
        ...
        Tensor gK = seqlen_traits_k.get_local_tile_tensor(
            mK, select<1, 2>(TileShape_MNK{}), bidh_kv, bidb);  // (N, K, _)
        Tensor gRab = local_tile(mRab(_, _, bidh_rab, bidb), select<0, 1>(TileShape_MNK{}),
                       make_coord(m_block, _)); // (M, N, _)
        Tensor gV = seqlen_traits_k.get_local_tile_tensor(
            mV, select<1, 2>(TileShape_MNK{}), bidh_kv, bidb);  // (N, K, _)
        ...
        if (warp_idx_in_warpgroup == 0 && lane_predicate) {
            ...
            if (Has_Rab){
                ...
                //     printf("ptr_Rab = %p\n", mainloop_params.ptr_Rab);
                //     printf("ptr_Rab[0:5] = %f, %f, %f, %f, %f\n", (float)mainloop_params.ptr_Rab[0], (float)mainloop_params.ptr_Rab[1], (float)mainloop_params.ptr_Rab[2], (float)mainloop_params.ptr_Rab[3], (float)mainloop_params.ptr_Rab[4]);
                // }
            }
        }
    ...
    mma(Params const& mainloop_params,
        ...
        int const actual_seq_q = seqlen_traits_q.actual_seq_len;
        int const actual_seq_k = seqlen_traits_k.actual_seq_len;
        int const actual_seq_h = Is_target ? seqlen_traits_k.actual_seq_len_h : actual_seq_k;
        int const actual_seq_c = Is_context ? seqlen_traits_k.actual_seq_len_c : 0;
        const int actual_seq_offset = Is_delta_q ? actual_seq_k - actual_seq_q : 0;
        
        int const max_seq_len = seqlen_traits_q.max_seq_len; // div seqlen_q
        ...
        auto col_limit_right = [&](int row, int n_block) {
            return std::min(
                actual_seq_k,
                row + 1 + actual_seq_offset + mainloop_params.window_size_right
            );
        };
        auto col_limit_left = [&](int row, int n_block) {
            return std::max(
                0,
                row + actual_seq_offset - mainloop_params.window_size_left
            );
        };
            ...
            for (int i = 0; i < size(tensor); ++i) {
                ...
                if constexpr (!Is_causal && !Is_local) { 
                    ...
                    if constexpr (Is_target && !Is_delta_q) { // because the definition is unclear
                        if (row >= actual_seq_h) {
                            const int target_index = (row - actual_seq_h) / mainloop_params.target_group_size;
                            const int target_col_limit_left = actual_seq_h + target_index * mainloop_params.target_group_size;
        ...
        Tensor tSsRab = smem_thr_copy_rab.partition_S(sRab); // (CPY, CPY_M, CPY_N, PIPE)
        Tensor tSrRab = make_tensor<Element>(partition_shape_C(tiled_mma0, select<0, 1>(TileShape_MNK{})));
        Tensor tSrRab_view = smem_thr_copy_rab.retile_D(tSrRab); // (CPY, CPY_M, CPY_N)
        ...
        if constexpr (IntraWGOverlap) {
            ...
            for (int masking_step = 0; n_block > n_block_min; ++masking_step,--n_block) {
                ...
                const bool is_masking = masking_step < n_masking_steps - 1 || (n_block + 1) * kBlockN > actual_seq_h; // latter for sparse masking
                if (Has_Rab) {
                    consumer_wait(pipeline_rab, smem_pipe_read_rab);
                    cute::copy(smem_tiled_copy_rab, tSsRab(_, _, _, smem_pipe_read_rab.index()), tSrRab_view(_, _, _));
                    cute::copy(make_tensor(convert_type<ElementAccum>(tSrRab).data(), tSrS.layout()), tSrS);
            ...
            #pragma unroll 1
            for (int masking_step = 0; n_block >= n_block_min; ++masking_step,--n_block) {
                const bool is_masking = masking_step < n_masking_steps || (n_block + 1) * kBlockN > actual_seq_h; // latter for sparse masking
                if (Has_Rab) {
                    consumer_wait(pipeline_rab, smem_pipe_read_rab);
                    cute::copy(smem_tiled_copy_rab, tSsRab(_, _, _, smem_pipe_read_rab.index()), tSrRab_view(_, _, _));
                    cute::copy(make_tensor(convert_type<ElementAccum>(tSrRab).data(), tSrS.layout()), tSrS);
    ...
    mma_fp8(Params const& mainloop_params,
        ...
        int const max_seq_len = seqlen_traits_q.max_seq_len;
        ...
        if (Has_Rab) {
            ...
            cutlass::arch::NamedBarrier::sync(cutlass::NumThreadsPerWarpGroup, static_cast<int>(FwdNamedBarriers::AddRabWG1) - 1 + cutlass::canonical_warp_group_idx() /*id*/);
        ...
        auto col_limit_right = [&](int row, int n_block) {
            int col_limit_base = row + 1 + seqlen_k - n_block * kBlockN - seqlen_q + m_block * kBlockM;
            if constexpr(Is_local)
        ...
        if constexpr (Is_causal) {
            ...
            for (int iter = 0; iter < extra_iterations && n_block >= 0; ++iter, --n_block) {
                ...
                if (Has_Rab) {
                    ...
                    cutlass::arch::NamedBarrier::sync(cutlass::NumThreadsPerWarpGroup, static_cast<int>(FwdNamedBarriers::AddRabWG1) - 1 + cutlass::canonical_warp_group_idx() /*id*/);
                    if constexpr(!Delay_V_release) { pipeline_rab.consumer_release(smem_pipe_read); }
                }
                for (int i = 0; i < size(tSrS); ++i) {
                    tSrS(i) *= mainloop_params.alpha;
                }
                ...
                if constexpr (Delay_V_release) { pipeline_k.consumer_release(smem_pipe_read); pipeline_rab.consumer_release(smem_pipe_read); }
                else { consumer_wait(pipeline_vt, smem_pipe_read); }
                flash::gemm</*zero_init=*/false, /*wg_wait=*/0>(tiled_mma1, tOrP, tOrV(_, _, _, smem_pipe_read.index()), tOrO);
                if constexpr(!Delay_V_release) { pipeline_vt.consumer_release(smem_pipe_read); }                
                ++smem_pipe_read;
