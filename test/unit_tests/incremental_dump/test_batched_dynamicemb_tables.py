...
@pytest.mark.parametrize("score_strategy", [
  DynamicEmbScoreStrategy.TIMESTAMP,
  DynamicEmbScoreStrategy.STEP,
  DynamicEmbScoreStrategy.CUSTOMIZED])
@pytest.mark.parametrize("pooling_mode", [
  DynamicEmbPoolingMode.SUM,
  DynamicEmbPoolingMode.NONE])
@pytest.mark.parametrize("table_num, features_per_table, num_embeddings, dim", [
  pytest.param(1, [1], [BATCH_SIZE_PER_DUMP * 8], 8, # large capacity to test dump in loops.
               id="No eviction"),
])
...
def test_without_eviction(request, current_device, score_strategy, pooling_mode, 
                          table_num, features_per_table, num_embeddings, dim, 
                          bucket_capacity, local_batch, num_iteration, dump_interval,
                          fixed_hot_size):
  print(f"\n{request.node.name}")
  options_list = [
    DynamicEmbTableOptions(
  ...
  model = BatchedDynamicEmbeddingTables(
    ...
    feature_table_map = [i for i, feature_num in enumerate(features_per_table) for _ in range(feature_num)],
  ...
  if score_strategy != DynamicEmbScoreStrategy.CUSTOMIZED:
    ...
    for i in range(0, num_iteration, dump_interval):
      ...
      for j in range(dump_interval):
        indices, offsets = _generate_sparse_feature(local_batch, unique_indices, device)
