...
@pytest.mark.parametrize("table_num, num_embeddings, use_dynamicembs, score_strategies, multi_hot_sizes", [
  pytest.param(1, [BATCH_SIZE_PER_DUMP * 8], [True], [DynamicEmbScoreStrategy.TIMESTAMP], [10]),
  pytest.param(4, [BATCH_SIZE_PER_DUMP * 8] * 4, [True] * 4, [DynamicEmbScoreStrategy.STEP, DynamicEmbScoreStrategy.TIMESTAMP, DynamicEmbScoreStrategy.CUSTOMIZED, DynamicEmbScoreStrategy.STEP], [10] * 4),
  pytest.param(4, [BATCH_SIZE_PER_DUMP * 8] * 4, [False, False, False, False], [None, None, None, None], [10] * 4),
  pytest.param(4, [BATCH_SIZE_PER_DUMP * 8] * 4, [True, False, True, False], [DynamicEmbScoreStrategy.STEP, None, DynamicEmbScoreStrategy.CUSTOMIZED, None], [10] * 4),
  pytest.param(4, [BATCH_SIZE_PER_DUMP * 8] * 4, [True] * 4, [DynamicEmbScoreStrategy.CUSTOMIZED] * 4, [10] * 4),
])
@pytest.mark.parametrize("is_pooled, pooling_mode", [
...
def test_incremental_dump_api(
  ...
  planner = get_planner(table_names, eb_configs, use_dynamicembs, score_strategies, 
                        local_batch, multi_hot_sizes, device)
  ...
  all_customized = True if score_strategies == [DynamicEmbScoreStrategy.CUSTOMIZED] * table_num else False
  ...
  for i in range(0, num_iteration):
    ...
    for j in range(dump_interval):
      ...
      sparse_feature = generate_sparse_feature(feature_names, multi_hot_sizes, local_batch, 
                                               unique_indices, use_dynamicembs, num_embeddings)
      ret = model(sparse_feature) # => this is awaitable
      kt = ret.values() # wait
    
    print("Dump socre=", undump_score)
